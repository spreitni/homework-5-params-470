...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname |homework 9|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
;***** Environment Stuff *****
(define resolve
  (lambda (varName env)
    (cond
    ...
    (cond
      ...
      ((eq? (car parsed-no-code) 'ask-exp)
       (if (run-parsed-boolean-code (cadr parsed-no-code) env) 
           ...
           (run-parsed-code (cadddr parsed-no-code) env)))
      (else
       (if(eq? (car parsed-no-code) (caar(cdadr(cdadr parsed-no-code))))
          (display(run-parsed-code (car(cdadr(cdadr parsed-no-code))) env))
